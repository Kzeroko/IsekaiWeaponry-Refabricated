...
public class PanDoraR extends SwordItem {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (user instanceof PlayerEntity) {
            ...
            ItemStack stack = new ItemStack(IsekaiWeapons.PAN_DORAL);
            if (player.getEquippedStack(EquipmentSlot.OFFHAND).isItemEqual(stack)) {
                if (!player.getItemCooldownManager().isCoolingDown(this) && !player.getItemCooldownManager().isCoolingDown(IsekaiWeapons.PAN_DORAL)) {
                    if (!world.isClient)
                    {
    ...
    public void appendTooltip(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
        ...
        tooltip.add(Text.translatable("desc.isekaiweaponryfabric.pan_dorar").formatted(Formatting.GOLD, Formatting.BOLD));

        if (Screen.hasShiftDown()) {
            tooltip.add(Text.translatable("desc.isekaiweaponryfabric.pan_dorar_story").formatted(Formatting.ITALIC));
        } else {
