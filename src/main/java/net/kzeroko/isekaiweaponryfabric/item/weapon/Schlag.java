...
public class Schlag extends RangeWeapon {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (user instanceof PlayerEntity) {
            ...
            ItemStack stack1 = new ItemStack(IsekaiRangeWeapons.SCHLAG);
            ItemStack stack2 = new ItemStack(IsekaiRangeWeapons.DONNER);
    ...
    public void reload(PlayerEntity user, Hand hand) {
        if (user.getStackInHand(hand).getItem() instanceof Schlag) {
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if (world.isClient) {
            if (((PlayerEntity) entity).getMainHandStack().getItem() instanceof Schlag
                    && IsekaiweaponryClient.reload.isPressed() && selected) {
                PacketByteBuf passedData = new PacketByteBuf(Unpooled.buffer());
                passedData.writeBoolean(true);
                ClientPlayNetworking.send(IsekaiweaponryFabric.SCHLAG, passedData);
            }
        }
    }
