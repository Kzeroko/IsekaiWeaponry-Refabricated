...
public class OrigamiMomiji extends SwordItem {
    public OrigamiMomiji(ToolMaterial toolMaterial, float attackSpeed, Settings settings) {
        super(toolMaterial, IsekaiweaponryFabric.config().origamimomijiDamage, attackSpeed, settings);
    }
    ...
    public void appendTooltip(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
        ...
        tooltip.add(Text.translatable("desc.isekaiweaponryfabric.origami_momiji").formatted(Formatting.GOLD, Formatting.BOLD));

        if (Screen.hasShiftDown()) {
            tooltip.add(Text.translatable("desc.isekaiweaponryfabric.origami_momiji_story").formatted(Formatting.ITALIC));
        } else {
