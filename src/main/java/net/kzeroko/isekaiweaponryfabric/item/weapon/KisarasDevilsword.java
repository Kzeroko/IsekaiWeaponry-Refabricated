...
public class KisarasDevilsword extends SwordItem {
    public KisarasDevilsword(ToolMaterial toolMaterial, float attackSpeed, Settings settings) {
        super(toolMaterial, MidnightConfigConstructor.kisaras_devilsword_damage, attackSpeed, settings);
    }

    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ServerWorld world = (ServerWorld) attacker.world;
        if (attacker instanceof ServerPlayerEntity && target.getGroup() == EntityGroup.UNDEAD) {
            target.damage(DamageSource.MAGIC, MidnightConfigConstructor.kisaras_devilsword_damage + MidnightConfigConstructor.kisaras_devilsword_extradamage);
    ...
    public void appendTooltip(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
        ...
        tooltip.add(Text.translatable("desc.isekaiweaponryfabric.kisaras_devilsword").formatted(Formatting.GOLD, Formatting.BOLD));

        if (Screen.hasShiftDown()) {
            tooltip.add(Text.translatable("desc.isekaiweaponryfabric.kisaras_devilsword_story").formatted(Formatting.ITALIC));
        } else {
